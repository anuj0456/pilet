...
class VideoRecorder:
    ...
    def __init__(self, name="temp_video.avi", fourcc="MJPG", sizex=640, sizey=480, camindex=0, fps=30):
        ...
        self.frameSize = (sizex, sizey)  # video formats and sizes also depend on and vary according to the camera used
        self.video_filename = name
        self.video_cap = cv2.VideoCapture(self.device_index)
        self.video_writer = cv2.VideoWriter.fourcc(*self.fourcc)
    ...
    def start(self):
        ...
        video_thread = threading.Thread(target=self.watch)
        video_thread.start()
...
class AudioRecorder:
    ...
    def start(self):
        ...
        audio_thread = threading.Thread(target=self.listen)
        audio_thread.start()


def start_av_recording(filename="test"):
    global video_thread
    global audio_thread
